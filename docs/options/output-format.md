...
Вы можете переопределить отдельные параметры конфигурации для каждого формата вывода, указав объект `format` в файле конфигурации. Это позволяет настраивать целевое окружение (`target`) и другие параметры индивидуально для каждого формата.
