...
Вы можете переопределить отдельные параметры конфигурации для каждого формата вывода, указав объект `format` в файле конфигурации. Это позволяет настраивать окружение сборки (`target`) и другие параметры индивидуально для каждого формата.
