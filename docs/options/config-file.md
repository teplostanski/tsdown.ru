...
## Указание пользовательского файла конфигурации {#указание-пользовательского-файла-конфигурации}
...
## Отключение файла конфигурации {#отключение-файла-конфигурации}
...
- `auto` (по умолчанию): использует встроенную поддержку TypeScript в среде выполнения (если она есть), в противном случае переключается на загрузчик `unrun`.
...
- `unrun`: Загружает файлы конфигурации с помощью библиотеки [`unrun`](https://gugustinette.github.io/unrun/). Это обеспечивает более мощные и гибкие возможности загрузки.

> [!TIP]
> Node.js по умолчанию не поддерживает импорт файлов TypeScript без указания расширения `.ts`. Если используется Node.js и нужно загрузить файл конфигурации TypeScript без расширения, рекомендуется использовать загрузчик `unrun` для удобства и совместимости.
