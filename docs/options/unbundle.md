...
## Управление корнем вывода

По умолчанию структура директорий в выводе определяется общей базовой директорией всех файлов точек входа. Чтобы переопределить это поведение, используйте опцию [`root`](/options/root):
...
export default defineConfig({
  entry: ['src/index.ts'],
  unbundle: true,
  root: '.', // Сохраняет префикс `src/` в выводе
})
```
