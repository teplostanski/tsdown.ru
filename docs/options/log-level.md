...
Опция `failOnWarn` задаёт, приводят ли предупреждения к завершению сборки с ненулевым кодом выхода.
...
export default defineConfig({
  failOnWarn: true,
  // Завершать сборку с ошибкой при предупреждениях в CI
  failOnWarn: 'ci-only',
})
```

Подробнее об опциях с учётом CI — в разделе [CI окружение](/advanced/ci).
