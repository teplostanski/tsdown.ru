...
Флаги CLI можно писать и в camelCase, и в kebab-case: например, `--outDir` и `--out-dir` означают одно и то же.
...
См. также [Отключение файла конфигурации](../options/config-file.md#отключение-файла-конфигурации).
...
::: warning Устарело
Используйте вместо этого `--log-level error`.
:::

Отключает вывод всех логов, кроме ошибок, во время процесса сборки. На экране будут показаны только сообщения об ошибках.
...
Задаёт платформу (окружение выполнения) для бандла. Варианты:
...
См. также [Валидация пакета](../options/lint.md).

## `--attw`

Включает интеграцию с [Are the types wrong?](https://github.com/arethetypeswrong/arethetypeswrong.github.io): проверяет TypeScript-типы вашего пакета на проблемы совместимости.
...
**[экспериментально]** Упаковка в автономный исполняемый файл с помощью [Single Executable Applications в Node.js](https://nodejs.org/api/single-executable-applications.html).

Эта опция упаковывает результат сборки в единый исполняемый файл. Требуется Node.js версии 25.7.0 или выше; не поддерживается в Bun и Deno. Кроссплатформенная сборка доступна через пакет `@tsdown/exe`.
...
См. также [Исполняемый файл](../options/exe.md).

## `-W, --workspace [dir]`

Включает режим workspace для сборки нескольких пакетов в монорепозитории. При необходимости можно указать корневой каталог workspace.

## `-F, --filter <pattern>`

Фильтрует конфигурации по рабочему каталогу или имени пакета. Допускаются строковое сопоставление и регулярные выражения (например, `/pkg-name$/` или `pkg-name`).

## `--unbundle`

Включает режим unbundle (сборка без объединения в один бандл): каждый исходный файл компилируется отдельно, структура каталогов в выводе сохраняется.

См. также [Сборка без объединения](../options/unbundle.md).

## `--fail-on-warn`

Завершает сборку с ошибкой при появлении предупреждений. По умолчанию включено.

См. также [CI окружение](../advanced/ci.md).
