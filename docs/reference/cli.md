...
Эта опция упаковывает результат сборки в единый исполняемый файл. Требуется Node.js версии 25.5.0 или выше; не поддерживается в Bun и Deno. Кроссплатформенная сборка доступна через пакет `@tsdown/exe`.

Когда включён `exe`:

- Формат вывода по умолчанию меняется с `esm` на `cjs` (за исключением Node.js >= v25.7.0, где поддерживается ESM).
